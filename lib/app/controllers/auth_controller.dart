import 'package:chatapp/app/data/models/chats_model_model.dart';
      ...
      if (isSignIn) {
        ...
        if (checkuser.data() == null) {
          await users.doc(_currentUser!.email).set({
            ...
            "name": _currentUser!.displayName!.substring(0, 1).toUpperCase() +
                _currentUser!.displayName!.substring(1),
...
// PROFILE
  ...
  void changeProfile(String name, String status) {
    ...
    // Update model(udate digunakan untuk update sebagian)
    user.update((user) {
      user!.name = name;
  ...
  void addNewConnection(String friendEmail) async {
    ...
    if (flagNewConnection) {
      ...
      if (chatsDocs.docs.length != 0) {
        ...
        if (listChats.docs.length != 0) {
          List<Chats> dataListChats = List<Chats>.empty();
          listChats.docs.forEach((element) {
            var dataDocChat = element.data();
            var dataDocChatId = element.id;
            dataListChats.add(Chats(
